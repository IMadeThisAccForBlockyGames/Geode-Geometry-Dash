...
static std::tuple<QString, qint64> computeFileSize(const QFileInfo& file)
{
    if (file.isDir()) {
...
void Resource::parseFile()
{
    ...
    std::tie(m_size_str, m_size_info) = computeFileSize(m_file_info);
...
static void stripThePrefix(QString& string)
{
...
int Resource::compare(const Resource& other, SortType type) const
{
    switch (type) {
        ...
        case SortType::NAME: {
            ...
            stripThePrefix(this_name);
            stripThePrefix(other_name);
